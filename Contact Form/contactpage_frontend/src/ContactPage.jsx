...
const ContactPage = () => {
  ...
  //   if (sendCopy) {
  //     await axios.post(`${port}/api/sentMail`, inputs).then((res) =>{
  ...
  //     // Sheet-base API here
  //     await axios
  //     .post('https://sheetdb.io/api/v1/w0oohhboujtek', data)
  //     .then((res) => {
  //       console.log(res);
  ...
  const handleSubmit = async (e) => {
    try {
      console.log('Before SheetDB Request');
      e.preventDefault();
      setSendingData(true);
      ...
      // If email doesn't have a proper validation, alert will be displayed and returned
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(inputs.emailId)) {
        alert('Please enter a valid email address');
        setSendingData(false);
      ...
      // If email has to send then this function will be triggered
      if (sendCopy) {
        const sentMailResponse = await axios.post(`${port}/api/sentMail`, inputs);
        if (sentMailResponse.data.message === 'Wrong Email Address') {
          toast.error('Your Email Address is incorrect');
        }
      }
  
      // This try-catch block is to send data to Google sheets
      try {
        const data = {
          Name: inputs.name,
          EmailID: inputs.emailId,
        ...
        const sheetDbResponse = await axios.post('', data, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${AIzaSyAGS6745DUZiTvbjXbHT7581mDvufa4ww4}`,
          },
        });
  
        console.log(sheetDbResponse);
        setInputs({ name: '', emailId: '', phone: '', message: '' });
        ...
        console.error('Error submitting form data to SheetDB:', error);
        setSendingData(false);
      }
    } catch (error) {
